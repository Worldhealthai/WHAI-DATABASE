...
// to work through. Everything comes from the admin panel's line-up.
...
import { Avatar, Notice, Tone, consentLabel, speakerPostStatus, sponsorDue, sponsorRemaining, useMarketing, type SpeakerRow, type SponsorRow } from './shared'

export function MarketingOverview() {
  const { year, href } = useWorkspace()
  const sp = useMarketing<SpeakerRow>('speaker')
  const so = useMarketing<SponsorRow>('sponsor')
  const loading = sp.isLoading || so.isLoading
  const speakers = sp.data?.data ?? []
  const sponsors = so.data?.data ?? []

  const todo = speakers.filter((s) => speakerPostStatus(s) === 'To do')
  const posted = speakers.filter((s) => speakerPostStatus(s) === 'Posted')
  const consented = speakers.filter((s) => s.linkedin_consent === true)
  const owed = sponsors.filter((s) => sponsorRemaining(s) > 0)
  const owedPosts = owed.reduce((n, s) => n + sponsorRemaining(s), 0)
  const included = sponsors.reduce((n, s) => n + Number(sponsorDue(s) ?? 0), 0)

  const err = sp.data?.error || so.data?.error
  const migration = Boolean(sp.data?.migration || so.data?.migration)

  return (
    <WorkspacePage title="Overview" description="LinkedIn posts for this edition: the line-up's speakers to welcome and the sponsor packages to honour, as the admin panel holds them.">
      {err ? (
        <Notice message={err} tone={migration ? 'warn' : 'bad'} />
      ) : (
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            <div className="ws-card">
              ...
              {!loading && todo.length === 0 ? (
                <EmptyState icon={Megaphone} title="Everyone is welcomed" body={speakers.length ? 'Every consenting speaker has had their post.' : `No speakers on the ${year} line-up yet.`} className="py-10" />
              ) : (
                <ul>
                  {todo.slice(0, 8).map((s) => {
                    ...
                    return (
                      <li key={s.id} className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--line)' }}>
                        <Avatar src={s.image} name={s.name || '?'} size={34} />
                        <span className="min-w-0 flex-1">
                          <span className="block font-medium truncate" style={{ color: 'var(--fg)' }}>{s.name}</span>
                          <span className="block text-[12px] truncate" style={{ color: 'var(--fg-3)' }}>{[s.role, s.org].filter(Boolean).join(' · ') || '—'}</span>
                        </span>
            ...
            <div className="ws-card">
              ...
              {!loading && owed.length === 0 ? (
                <EmptyState icon={Megaphone} title="Nothing owed" body={sponsors.length ? 'Every sponsor package has had its posts.' : `No sponsors on the ${year} event yet.`} className="py-10" />
              ) : (
                <ul>
                  {owed.slice(0, 8).map((s) => (
                    <li key={s.id} className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--line)' }}>
                      <Avatar src={s.logo} name={s.name || '?'} size={34} />
                      <span className="min-w-0 flex-1">
                        <span className="block font-medium truncate" style={{ color: 'var(--fg)' }}>{s.name}</span>
                        <span className="block text-[12px] truncate" style={{ color: 'var(--fg-3)' }}>{s.tier || '—'} · {s.tracking.postsDone ?? 0} of {sponsorDue(s)} made</span>
                      </span>
