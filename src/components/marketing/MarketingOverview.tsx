...
import { Avatar, MigrationNotice, Tone, consentLabel, speakerPostStatus, sponsorRemaining, useMarketing } from './shared'
...
export function MarketingOverview() {
  ...
  return (
    <WorkspacePage title="Overview" description="LinkedIn posts for this edition: speakers to welcome and sponsor packages to honour.">
      {error ? (
        <MigrationNotice message={error} />
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            <Stat label="Speakers to welcome" value={todo.length} loading={loading} hint={`${consented.length} of ${speakers.length} consented`} />
