...
function ContactCard({ contact }: { contact: Contact }) {
  return (
    <Link href={`/contacts/${contact.id}`} className="block px-4 py-3 border-b border-border hover:bg-[#112850]/50 transition-colors">
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-full bg-[#1a3a5c] flex items-center justify-center text-xs font-semibold text-[#00B4D8] shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <div className="w-10 h-1.5 rounded-full bg-[#1a3a5c] overflow-hidden">
                <div className="h-full rounded-full bg-[#00B4D8]" style={{ width: `${contact.engagementScore}%` }} />
              </div>
              <span className="text-[10px] text-slate-400">{contact.engagementScore}</span>
            </div>
          </div>
          <p className="text-xs text-slate-400 truncate mt-0.5">{contact.jobTitle}</p>
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            {contact.company && (
              <span className="text-[11px] text-slate-300">{contact.company.name}</span>
            )}
            {contact.seniority && (
              <span className={cn('text-[10px] px-1.5 py-0.5 rounded border', SENIORITY_COLORS[contact.seniority] ?? 'text-slate-300 bg-slate-700/50 border-slate-600')}>
                {contact.seniority}
              </span>
            )}
