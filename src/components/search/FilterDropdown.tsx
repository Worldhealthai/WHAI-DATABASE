...
export function FilterDropdown({
  ...
  const menu = open && typeof document !== 'undefined' && createPortal(
    <div
      ref={menuRef}
      style={menuStyle}
      className="bg-[var(--surface)] border border-[var(--line)] rounded-lg shadow-2xl shadow-black/60 overflow-hidden"
    >
        ...
        <div className="p-2 border-b border-[var(--line)]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-[var(--surface-2)] border border-[var(--line)] rounded text-xs text-[var(--fg)] placeholder-slate-500 outline-none focus:border-[var(--teal-line)]"
            />
          </div>
        </div>
      )}
              ...
              <button
                ...
                className={cn(
                  ...
                  isSelected
                    ...
                    : 'text-slate-300 hover:bg-[var(--surface-2)] hover:text-[var(--fg)]',
                )}
              >
                <div
                  className={cn(
                ...
                >
                  {isSelected && <Check className="w-3 h-3 text-[var(--on-accent)]" strokeWidth={3} />}
                </div>
      ...
      {hasSelected && (
        <div className="p-2 border-t border-[var(--line)] flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-slate-400 hover:text-[var(--fg)] transition-colors"
          >
            Clear
          </button>
        </div>
      ...
      <button
        ...
        className={cn(
          ...
          open
            ...
            : hasSelected
              ? 'bg-[var(--teal-soft)] text-[var(--teal)] border-[var(--teal-line)] hover:border-[var(--teal-line)]'
              : 'bg-[var(--surface-2)] text-slate-300 border-[var(--line)] hover:text-[var(--fg)] hover:border-slate-500',
        )}
      >
        {label}
        {hasSelected && (
          <span className="ml-0.5 px-1.5 py-0.5 rounded-full bg-[var(--teal)] text-[var(--on-accent)] text-[10px] font-bold leading-none">
            {selected.length}
          </span>
        )}
...
export function ActiveFiltersBar({ filters, onRemove, onClearAll }: ActiveFiltersBarProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-1 gap-y-1.5 py-3 border-b border-[var(--line)]">
        ...
        <div key={category} className="flex items-center gap-1">
          ...
          {items.map((item) => (
            <span
              ...
              className="inline-flex items-center gap-1 pl-2.5 pr-1.5 py-1 rounded-full text-xs font-medium bg-[var(--surface-2)] text-slate-200 border border-[var(--line)] hover:border-slate-500 transition-colors"
            >
              {item.display ?? item.value}
              <button
                onClick={() => onRemove(item.key, item.value)}
                className="ml-0.5 p-0.5 rounded-full hover:bg-slate-600/50 text-slate-400 hover:text-[var(--fg)] transition-colors"
              >
                <X className="w-3 h-3" />
              </button>
            </span>
      ...
      <button
        onClick={onClearAll}
        className="ml-2 text-xs text-slate-500 hover:text-[var(--teal)] transition-colors"
      >
        Clear all
      </button>
    </div>
