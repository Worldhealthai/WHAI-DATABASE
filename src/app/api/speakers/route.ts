...
import type { SpeakerFilters } from '@/types'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filters: SpeakerFilters = {
      ...
      expertiseAreas: searchParams.getAll('expertiseAreas').filter(Boolean),
    ...
    if (filters.query) {
      query = query.or(
        `firstName.ilike.%${filters.query}%,lastName.ilike.%${filters.query}%,email.ilike.%${filters.query}%,organization.ilike.%${filters.query}%,jobTitle.ilike.%${filters.query}%,sessionTitle.ilike.%${filters.query}%`
      )
    }
    ...
    if (filters.expertiseAreas?.length) {
      query = query.or(filters.expertiseAreas.map((e) => `expertiseAreas.ilike.%${e}%`).join(','))
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error?.code === '23505' || error?.message?.includes('duplicate key')) {
      return NextResponse.json(
        { error: 'This speaker already exists in your CRM — duplicate entries are not allowed.' },
        { status: 409 },
      )
    }
