...
// POST /api/partners/[id]/set-primary
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [{ data: company, error: companyErr }, { data: contact, error: contactErr }] = await Promise.all([
      supabase.from('partners').select('*').eq('id', params.id).is('companyId', null).single(),
      supabase.from('partners').select('*').eq('id', contactId).eq('companyId', params.id).single(),
    ])
    ...
    const { error: deleteErr } = await supabase.from('partners').delete().eq('id', contactId)
