    ...
    try {
      if (movingContact.isPrimary) {
        await fetch('/api/sponsors', {
          ...
          body: JSON.stringify({
            companyId: targetId, companyName: targetName, status: 'Active',
  ...
  if (error || !partner) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-16 text-center">
        ...
        <Link href="/partners" className="text-emerald-400 text-sm hover:underline">← Back to Partners</Link>
      </div>
    )
  }

  const tagList = partner.tags?.split(',').map((t) => t.trim()).filter(Boolean) ?? []
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <div className="whai-card overflow-hidden">
        <div className="h-0.5 w-full" style={{ background: `linear-gradient(90deg, ${ACCENT}80 0%, ${ACCENT}20 60%, transparent 100%)` }} />
        <div className="p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="w-14 h-14 rounded-xl flex items-center justify-center text-xl font-bold shrink-0" style={{ background: `${ACCENT}20`, color: ACCENT }}>
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              {partner.website && (
                <a href={partner.website} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs hover:opacity-80 transition-opacity"
              ...
              {partner.event && (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs"
                  style={{ background: `${ACCENT}08`, borderColor: `${ACCENT}20`, color: `${ACCENT}cc` }}>
                  {partner.event}
                </span>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="whai-card p-5">
            <h2 className="text-sm font-semibold text-white mb-4">Partnership Details</h2>
            <div className="grid grid-cols-2 gap-4">
              <DetailField label="Status" value={<StatusBadge value={partner.status} variant="sponsor_status" />} />
              {partner.tier && <DetailField label="Tier" value={<StatusBadge value={partner.tier} variant="sponsor_tier" />} />}
              <DetailField label="Event" value={partner.event ?? '—'} />
              ...
              <div className="space-y-3">
                ...
                {partner.contactEmail && (
                  <InfoRow icon={Mail} label="Email">
                    <a href={`mailto:${partner.contactEmail}`} className="hover:underline" style={{ color: ACCENT }}>{partner.contactEmail}</a>
                  </InfoRow>
                )}
                ...
                {partner.contactLinkedinUrl && (
                  <InfoRow icon={Linkedin} label="LinkedIn">
                    <a href={partner.contactLinkedinUrl} target="_blank" rel="noopener noreferrer" className="hover:underline" style={{ color: ACCENT }}>View Profile</a>
                  </InfoRow>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              {(partner.contactFirstName || partner.contactLastName || partner.contactEmail) && (
                <ContactCard
                  ...
                  jobTitle={partner.contactJobTitle} email={partner.contactEmail}
                  phone={partner.contactPhone} linkedinUrl={partner.contactLinkedinUrl}
                  accentHex={ACCENT} isPrimary
                  onEdit={() => setEditingContact({ id: partner.id, companyId: partner.id, contactFirstName: partner.contactFirstName, contactLastName: partner.contactLastName, contactEmail: partner.contactEmail, contactPhone: partner.contactPhone, contactJobTitle: partner.contactJobTitle, contactLinkedinUrl: partner.contactLinkedinUrl, notes: null, createdAt: partner.createdAt })}
                  onMove={() => setMovingContact({ isPrimary: true, contact: { id: partner.id, companyId: partner.id, contactFirstName: partner.contactFirstName, contactLastName: partner.contactLastName, contactEmail: partner.contactEmail, contactPhone: partner.contactPhone, contactJobTitle: partner.contactJobTitle, contactLinkedinUrl: partner.contactLinkedinUrl, notes: null, createdAt: partner.createdAt } })}
                />
              )}
              {partner.contacts?.map((c) => (
                <ContactCard key={c.id} accentHex={ACCENT}
                  name={[c.contactFirstName, c.contactLastName].filter(Boolean).join(' ')}
                  jobTitle={c.contactJobTitle} email={c.contactEmail}
                  phone={c.contactPhone} linkedinUrl={c.contactLinkedinUrl}
                  onEdit={() => setEditingContact(c)}
                  onMove={() => setMovingContact({ contact: c, isPrimary: false })}
                  onDelete={async () => { if (!confirm('Remove this contact?')) return; await fetch(`/api/sponsors/${c.id}`, { method: 'DELETE' }); refetch() }}
                />
              ))}
              {!partner.contactFirstName && !partner.contactEmail && (!partner.contacts || partner.contacts.length === 0) && (
      ...
      {addContactOpen && (
        <SponsorContactModal companyId={id} companyName={partner.companyName}
          onClose={() => setAddContactOpen(false)} onSaved={() => { setAddContactOpen(false); refetch() }} />
      )}
      {editingContact && (
        <SponsorContactModal companyId={id} companyName={partner.companyName} contact={editingContact}
          onClose={() => setEditingContact(null)} onSaved={() => { setEditingContact(null); refetch() }}
          onSetPrimary={() => { setEditingContact(null); refetch() }} />
      )}
      {editOpen && <SponsorFormModal sponsor={partner} onClose={() => setEditOpen(false)} onSaved={() => { setEditOpen(false); refetch() }} />}
      {movingContact && (
        <MoveContactModal
          contactName={[movingContact.contact.contactFirstName, movingContact.contact.contactLastName].filter(Boolean).join(' ') || 'Contact'}
          currentSponsorId={id} onClose={() => setMovingContact(null)}
          onMove={handleMoveContact} loading={moveLoading} accentHex={ACCENT}
        />
      )}
    </div>
  )
...
function ContactCard({ name, jobTitle, email, phone, linkedinUrl, isPrimary, accentHex, onEdit, onMove, onDelete }: {
  name: string; jobTitle?: string | null; email?: string | null; phone?: string | null
  linkedinUrl?: string | null; isPrimary?: boolean; accentHex: string
  onEdit?: () => void; onMove?: () => void; onDelete?: () => void
}) {
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-[#0A1628] border border-[#1a3a5c] hover:border-emerald-500/30 hover:bg-[#0d1f3a] transition-colors group/card">
      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: `${accentHex}20`, color: accentHex }}>
        {name ? name.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase() : '?'}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-white">{name || '—'}</span>
          {isPrimary && <span className="text-[10px] px-1.5 py-0.5 rounded-full border" style={{ background: `${accentHex}10`, color: `${accentHex}cc`, borderColor: `${accentHex}30` }}>Primary</span>}
        </div>
        {jobTitle && <div className="text-xs text-slate-500 mt-0.5">{jobTitle}</div>}
        <div className="flex items-center gap-3 mt-1.5 flex-wrap">
          {email && <a href={`mailto:${email}`} onClick={(e) => e.stopPropagation()} className="text-xs hover:underline" style={{ color: accentHex }}>{email}</a>}
      ...
      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover/card:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
        {onMove && <button onClick={onMove} className="text-slate-600 hover:text-blue-400 transition-colors p-1" title="Move to another company"><ArrowRightLeft className="w-3.5 h-3.5" /></button>}
        {onEdit && <button onClick={onEdit} className="text-slate-600 hover:text-emerald-400 transition-colors p-1"><Pencil className="w-3.5 h-3.5" /></button>}
        {onDelete && <button onClick={onDelete} className="text-slate-600 hover:text-red-400 transition-colors p-1"><Trash2 className="w-3.5 h-3.5" /></button>}
      </div>
    </div>
  )
}

function MoveContactModal({ contactName, currentSponsorId, onClose, onMove, loading, accentHex }: {
  contactName: string; currentSponsorId: string; accentHex: string
  onClose: () => void; onMove: (targetId: string, targetName: string) => void; loading: boolean
}) {
  ...
  const sponsors = (data?.data ?? []).filter((s) => s.id !== currentSponsorId && (!query.trim() || s.companyName.toLowerCase().includes(query.toLowerCase())))
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-8 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-md bg-[#0d2040] border border-[#1a3a5c] rounded-xl shadow-2xl mt-16" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a3a5c]">
          <div>
            <h2 className="text-sm font-semibold text-white flex items-center gap-2"><ArrowRightLeft className="w-4 h-4" style={{ color: accentHex }} /> Move Contact</h2>
        ...
        <div className="p-4 border-b border-[#1a3a5c]">
          <div className="relative">
            ...
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search companies…"
        ...
        <div className="max-h-72 overflow-y-auto p-2">
          {sponsors.length === 0 && <p className="text-sm text-slate-500 text-center py-6">{query ? 'No companies match.' : 'No other companies found.'}</p>}
          {sponsors.map((s) => (
            <button key={s.id} onClick={() => onMove(s.id, s.companyName)} disabled={loading}
              ...
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold shrink-0" style={{ background: `${accentHex}20`, color: accentHex }}>
        ...
        {loading && <div className="px-5 py-3 border-t border-[#1a3a5c] text-xs text-slate-400 text-center">Moving contact…</div>}
      </div>
    </div>
  )
}
