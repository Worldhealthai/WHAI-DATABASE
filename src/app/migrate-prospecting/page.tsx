...
export default function MigrateProspectingPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center p-8">
      <div className="w-full max-w-md bg-[var(--surface)] border border-[var(--line)] rounded-xl p-6 space-y-4">
        <h1 className="text-[var(--fg)] font-bold text-lg">One-time Migration</h1>
        ...
        <button
          ...
          className="w-full py-2.5 rounded-lg bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-[var(--on-accent)] font-semibold text-sm transition-colors"
        >
          {running ? 'Running…' : result?.results ? 'Done ✓' : 'Run Migration'}
        </button>
        {result && (
          <pre className="text-xs text-slate-300 bg-[var(--bg)] rounded-lg p-3 overflow-auto">
